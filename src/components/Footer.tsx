...
export const Footer = () => {
  ...
  const content = {
    ar: {
      ...
      description:
        'منصة التسويق الرقمي والذكاء الاصطناعي التي تساعدك على تحليل السوق وبناء استراتيجيات تسويقية فعالة باستخدام الذكاء الاصطناعي.',
      product: 'المنتج',
      productItems: [
    ...
    en: {
      ...
      description:
        'Digital marketing and AI platform that helps you analyze the market and build effective marketing strategies using artificial intelligence.',
      product: 'Product',
      productItems: [
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        >
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <div className={isRTL ? 'text-right' : 'text-left'}>
                <h3
                  className={`text-2xl font-bold text-white group-hover:text-blue-300 transition-colors duration-300 ${fontClass}`}
                >
                  {t.companyName}
                </h3>
            ...
            <p
              className={`text-gray-300 leading-relaxed text-sm ${fontClass} ${rtlTextClass}`}
            >
              {t.description}
            </p>
            ...
            <div>
              <h4
                className={`text-lg font-semibold mb-4 text-white ${fontClass} ${rtlTextClass}`}
              >
                {t.followUs}
              </h4>
              <div
          ...
          <div className={`space-y-6 ${rtlTextClass}`}>
            <h4
              className={`text-xl font-bold text-white relative ${fontClass} ${rtlTextClass}`}
            >
              {t.quickLinks}
              <div
                className={`absolute bottom-0 w-12 h-0.5 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full ${rtlPositionClass}`}
              ></div>
            </h4>
            <ul className="space-y-3" role="list">
              {t.productItems.map((item, index) => (
                <li key={index}>
                  ...
                  >
                    <ArrowRight
                      className={`w-4 h-4 text-blue-400 opacity-0 group-hover:opacity-100 transition-all duration-300 ${
                        isRTL
                    ...
                    <span className="relative">
                      {item.name}
                      <span
                        className={`absolute bottom-0 w-0 h-0.5 bg-blue-400 transition-all duration-300 group-hover:w-full ${rtlPositionClass}`}
                      ></span>
                    </span>
                  </Link>
                </li>
          ...
          <div className={`space-y-6 ${rtlTextClass}`}>
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={index}
                  className={`group flex items-start gap-4 p-3 rounded-xl bg-gray-800/30 backdrop-blur-sm border border-gray-700/30 hover:border-gray-600/50 transition-all duration-300 `}
                >
                  <div
                    className={`flex-shrink-0 p-2 rounded-lg bg-gray-700/50 ${color}`}
                  >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            <div className={`flex items-center gap-2 ${rtlFlexClass}`}>
              ...
              <p
                className={`text-sm text-gray-400 ${fontClass} ${rtlTextClass}`}
              >
                © 2024{' '}
            ...
            <nav
              className={`flex gap-8 ${rtlFlexClass}`}
              role="navigation"
              aria-label="Footer legal links"
            >
              {[
                {
                  text: language === 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy',
                  href: '/privacy',
                },
                {
                  text:
                ...
                {
                  text: language === 'ar' ? 'ملفات تعريف الارتباط' : 'Cookies',
                  href: '/cookies',
                },
              ].map((link, index) => (
                <Link
