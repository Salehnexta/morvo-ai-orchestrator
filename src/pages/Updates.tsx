          ...
          <div className="space-y-8">
            {t.updates.map((update, index) => (
              <Card key={index} className={`${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white'}`}>
                  ...
                  <div className={`flex items-start gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className="flex-1">
                      <div className={`flex items-center gap-4 mb-2 ${isRTL ? 'flex-row-reverse justify-end' : 'justify-start'}`}>
                        ...
                        <div className={`flex items-center gap-2 text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} ${isRTL ? 'flex-row-reverse' : ''}`}>
                          <Calendar className="w-4 h-4" />
                          <span>{update.date}</span>
                        </div>
                      </div>
                      <CardTitle className={`text-xl mb-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                        {update.title}
                      </CardTitle>
                      <CardDescription className={`text-base ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                        {update.description}
                      </CardDescription>
                    </div>
                  </div>
                ...
                <CardContent>
                  <div className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} ${isRTL ? 'text-right' : 'text-left'}`}>
                    {update.version}
                  </div>
                </CardContent>
              </Card>
